<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="username" label="角色名称" sortable>
      </el-table-column>
      ...
      <el-table-column label="操作" width="300">
        <template slot-scope="scope">
          <el-button type="success" @click="update(scope.row.uid)"
            >修改</el-button
          >
          <el-button type="danger" @click="deletes(scope.row.uid)"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  mounted() {
    getUsercount().then((d) => {
      this.totals = Math.ceil(d.data.list[0].total / this.counts.size);
    });
  },
  methods: {
    deletes(uid) {
